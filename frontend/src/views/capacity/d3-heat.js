...
const MyD3Heatmap = ({ capacity }) => {
    ...
    const data2 = capacity ? capacity.map(item => [Number(item.processCd) / 10, Number(item.firmPsFacTp), item.remainQty]) : [];
    const reshapedData2 = data2.map(([x, y, value]) => [x, y, value]);
    ...
    //remain
    reshapedData2.forEach(([x, y, value]) => {
        resultArray[x - 1][y - 1] = value;
    });

    const cleanedResultArray2 = resultArray.map(arr => arr.slice(0, -1));
    const transformedData2 = cleanedResultArray2;
    ...
    const [clickedCell, setClickedCell] = useState({ value: null, index: null });
    ...
    useEffect(() => {
        ...
        const cells = svg.selectAll('.cell')
            ...
            .on('click', (event, d) => {
                const xCoord = event.clientX - svg.node().getBoundingClientRect().left - 165;
                const yCoord = event.clientY - svg.node().getBoundingClientRect().top - 110;

                const col = Math.floor(xCoord / xSize);
                const row = Math.floor(yCoord / ySize);
