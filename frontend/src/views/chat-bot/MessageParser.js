...
const MessageParser = ({ children, actions }) => {
  ...
  const parse = (message) => {
    ...
    if (
      ((preMessage.includes("가능통과공장") && message.includes("확정")) ||
        (message.includes("가능") && message.includes("확정"))) &&
      !message.includes("취소") &&
      !message.includes("코드")
    ) {
      messageFlag = true;
      setPremessage("확정처리");
      actions.handleTypingMessage(
        "가틍통과공장이 설계된 주문 건을 확정하는 것으로 확정처리 할 주문들을 선택하여 오른쪽 위 확정처리 버튼을 클릭하여 확정합니다."
      );
    }

    if (
      (message.includes("취소") && preMessage.includes("확정")) ||
      (message.includes("취소") && message.includes("확정"))
    ) {
      messageFlag = true;
    ...
    if (
      ...
      setPremessage("출강주");
      actions.handleTypingMessage(
        "출강주란 한 주의 주문을 받아서 처리하는 단위입니다."
      );
    }

    if (
      ((message.includes("현재") || message.includes("지금")) &&
        preMessage.includes("출강주")) ||
      ((message.includes("현재") || message.includes("지금")) &&
        message.includes("출강주"))
    ) {
      messageFlag = true;
      actions.handleTypingMessage(
        "현재 출강주는 다음과 같습니다. '20240130', '20240123', '20240116', '20240109', '20240102' "
      );
    }
    ...
    if (
      message.includes("부여") &&
      !(message.includes("현황") || message.includes("수정"))
    ) {
      messageFlag = true;
      setPremessage("공장부여");
      actions.handleTypingMessage(
        "공장 부여 버튼을 누를 경우 선택 주문에 대하여 투입능력관리에서 확인한 잔여 공장 능력이 가장 높은 공장으로 확정 통과 공장을 지정합니다."
      );
    }
    ...
    if (message.includes("부여") && message.includes("수정")) {
      ...
      setPremessage("공장부여");
      actions.handleTypingMessage(
        "밑에서 결정된 공장을 확인하고 여유 능력치를 체크하여 공장을 수기로 변경할 수 있습니다."
      );
    }
    ...
    if (
      (message.includes("취소") && preMessage.includes("제조투입")) ||
      (message.includes("취소") && message.includes("투입"))
    ) {
      messageFlag = true;
    ...
    if (
      (message.includes("공장") || message.includes("결과")) &&
    ...
    if (
      ((message.includes("공장") || message.includes("결과")) &&
        (message.includes("조정") || message.includes("변경"))) ||
      preMessage.includes("결과조정")
    ) {
      messageFlag = true;
      setPremessage("결과조정");
      if (message.includes("사용량")) {
        actions.handleTypingMessage(
          "능력 사용량은 공장에서 현재 사용중인 양 입니다."
        );
      }

      if (message.includes("이용량")) {
        actions.handleTypingMessage(
          "능력 이용량은 공장에서 현재 사용할 수 있는 양 입니다."
        );
      }
    }
    ...
    if (
      ...
      message.includes("능력") &&
      !(
        message.includes("히트맵") ||
        (message.includes("페이지") && message.includes("오른쪽")) ||
    ...
    if (
      (preMessage.includes("투입능력") ||
        (message.includes("투입") && message.includes("능력"))) &&
      (message.includes("히트맵") ||
        (message.includes("페이지") && message.includes("오른쪽")) ||
        (message.includes("오른쪽") && message.includes("그래프")) ||
        (message.includes("공장") &&
      ...
      setPremessage("투입능력");
      actions.handleTypingMessage(
        "공장의 부하 정도를 나타내는 히트맵으로 공장의 현재 부하 상태를 나타내었습니다. 색이 진할 수록 잔여량이 적어서 부하가 심한 상태임을 한눈에 확인할 수 있습니다."
      );
    }
    ...
    if (
      (message.includes("사이즈") ||
        (message.includes("사이즈") && message.includes("기준"))) &&
      !message.includes("수정") &&
      !message.includes("임시") &&
      !message.includes("설계") &&
      !message.includes("추가") &&
    ...
    if (
      (message.includes("사이즈") && message.includes("수정")) ||
      (preMessage.includes("사이즈기준") && message.includes("수정"))
    ) {
      messageFlag = true;
      actions.handleTypingMessage(
        "사이즈 기준 수정은 수정하려는 값을 더블클릭하여 수정 후 저장 버튼을 클릭하여 수정합니다.\n\n" +
    ...
    if (
      (message.includes("사이즈") && message.includes("추가")) ||
    ...
    if (
      (message.includes("사이즈") && message.includes("삭제")) ||
    ...
    if (
      (message.includes("사이즈") &&
        message.includes("임시") &&
        message.includes("설계")) ||
      (preMessage.includes("사이즈기준") &&
        message.includes("임시") &&
        message.includes("설계"))
    ) {
      messageFlag = true;
      actions.handleTypingMessage(
        "사이즈 기준 임시 설계는 두께와 폭, 길이, 단중을 입력하면 수치에 따른 공정별 공장을 보여줍니다."
      );
    }
    ...
    if (
      ((message.includes("필수재") && message.includes("기준")) ||
        preMessage.includes("필수재기준")) &&
      (message.includes("설명") ||
        message.includes("더 알려줘") ||
        message.includes("로직"))
    ) {
      messageFlag = true;
      setPremessage("필수재기준");
    ...
    if (
      ...
      actions.handleTypingMessage(
        "공정별로 가통 코드에 대응하는 공장 리스트를 확인할 수 있습니다."
      );
    }
    ...
    if (
      ...
      actions.handleTypingMessage(
        "공장마다 가능한 공정들을 확인할 수 있습니다."
      );
    }

    if (
      ((message.includes("가능") && message.includes("코드")) ||
        preMessage.includes("가능통과공장코드")) &&
      message.includes("수정") &&
      !message.includes("확정")
    ) {
      messageFlag = true;
      actions.handleTypingMessage(
        "변경하고자 하는 코드를 더블클릭하면 현재 선택된 공장만 체크 된 상태로 보여집니다. \n" +
    ...
    if (
      ((message.includes("확정") && message.includes("코드")) ||
        preMessage.includes("확정통과공장코드")) &&
      message.includes("수정")
    ) {
      messageFlag = true;
    ...
    if (
      ...
      actions.handleTypingMessage(
        "경유 공정 기준은 품명 별 필요한 공정들을 확인할 수 있습니다."
      );
    }

    if (
      ((message.includes("경유") && message.includes("공정")) ||
        preMessage.includes("경유공정기준")) &&
      message.includes("추가")
    ) {
      messageFlag = true;
      setPremessage("경유공정기준");
      actions.handleTypingMessage(
        "구분, 품종 및 품명을 입력한 후 경유가 가능한 공정을 선택하여 기준을 추가합니다."
      );
    }
    ...
    if (!messageFlag) {
      // setPremessage("");
      actions.handleTypingMessage(
        "죄송합니다. 잘 모르겠습니다. 도메인에 일치하는 검색어로 말씀주시면 친절히 답변드리겠습니다."
      );
    }
  };
