...
//    }
    ...
    public ResponseEntity<ResponseDto> possibleDecision(@RequestBody FactoryOrderInfoReqDto.decisionDto reqDto) {
        // 반환값 : 성공한 주문 개수, 실패한 주문 개수
        int successCnt = 0;
        int failCnt = 0;

        for (Long id : reqDto.getIds()) {
            boolean success = factoryOrderInfoService.possibleFactory(reqDto.getUserName(), id);
    ...
    public ResponseEntity<ResponseDto> confirmDecision(@RequestBody FactoryOrderInfoReqDto.decisionDto reqDto) {
        // 반환값 : 성공한 주문 개수, 실패한 주문 개수
        int successCnt = 0;
        int failCnt = 0;

        for (Long id : reqDto.getIds()) {
            boolean success = factoryOrderInfoService.confirmFactory(reqDto.getUserName(), id);
            if (success) successCnt++;
            else failCnt++;
        }
