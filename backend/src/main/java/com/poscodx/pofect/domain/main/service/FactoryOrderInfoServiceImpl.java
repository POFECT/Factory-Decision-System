...
//        String passResult = processStandardService.getByOrdPdtItdsCdN(order.getOrdPdtItdsCdN());
        ...
        if("00000000".equals(passResult)) {
            ...
            /** 필수재 설계 */
//                    필수재 함수 (processList, order.getId())
            ...
            for(SizeStandardSetDto s: essentialResult) {
            ...
            for (int i = 0, j = 10; i < 8; i++, j += 10) {
                ...
                // 공정에 해당하면 필수재, 사이즈 기준 교집합 추출
                    ...
                    SizeStandardSetDto essential = null;
                    SizeStandardSetDto size = null;

                    for (SizeStandardSetDto d : essentialResult) {
                    ...
                    // 필수재 X, 사이즈 X -> FF
                    if(essential.getFirmPsFacTpList().isEmpty() && size.getFirmPsFacTpList().isEmpty()) {
                        possibleCode.append("FF");
                        continue;
                    }
                    // 필수재 X, 사이즈 O -> FP
                    if(essential.getFirmPsFacTpList().isEmpty()) {
